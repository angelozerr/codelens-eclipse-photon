 ...
 * @since 3.13.0
 */
public class InlinedAnnotationSupport2 implements StyledTextLineSpacingProvider {
	...
	class FontPropertyChangeListener implements IPropertyChangeListener {
		...
		public void propertyChange(PropertyChangeEvent event) {
			if (fViewer == null || fInlinedAnnotations == null || fInlinedAnnotations.isEmpty())
				return;
			...
			List<AbstractInlinedAnnotation> lineContentAnnotations = fInlinedAnnotations.stream()
					.filter(ann -> ann instanceof LineContentAnnotation).collect(Collectors.toList());
			if (lineContentAnnotations.isEmpty()) {
				return;
			}
			try {
				...
				for (AbstractInlinedAnnotation ann : fInlinedAnnotations) {
					...
					if (style != null && style.metrics != null) {
						...
						style.metrics.width = ((LineContentAnnotation) ann).getWidth();
	...
	public Integer getLineSpacing(int lineIndex) {
		...
		return (annotation instanceof LineHeaderAnnotation) ? ((LineHeaderAnnotation) annotation).getHeight() : null;
	}

	/**
	 * Returns the {@link AbstractInlinedAnnotation} from the given line index and
	 * null otherwise.
	 *
	 * @param viewer
	 *            the source viewer
	 * @param lineIndex
	 *            the line index.
	 * @return the {@link AbstractInlinedAnnotation} from the given line index and
	 *         null otherwise.
	 */
